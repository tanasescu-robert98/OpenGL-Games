	...
	class Camera
	{
		public:
			...
			void stateCamera()
			{
				if (camera_mode == 0)
				{
					position = position - glm::normalize(forward) * distanceToTarget;
					RotateFirstPerson_OX(-M_PI / 10);
					camera_mode = 1;
				}
				else
				{
			...
			void TranslateRight(float distance)
			{
				...
				glm::vec3 dir2 = glm::normalize(glm::vec3(right.x, right.y, right.z));
				position = position + glm::normalize(dir2) * distance;
			}
